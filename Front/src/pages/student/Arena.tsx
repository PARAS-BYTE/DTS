...
const BattleSetup = () => {
  ...
  return (
    <div className="min-h-screen bg-[#0c0c0c] flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div
        ref={introContainer}
        className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black pointer-events-none"
      >
        ...
        <div
          ref={novaTextRef}
          className="mt-[100px] text-white text-4xl md:text-5xl font-extrabold text-center"
          aria-hidden
        />
      </div>
      ...
      <div className="w-full max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <motion.div
            ...
            className="rounded-2xl border border-gray-700 shadow-2xl bg-[#141414] p-6"
          >
            <Card className="bg-transparent shadow-none">
              ...
              <CardContent className="space-y-5">
                {!createdBattle ? (
                  <>
                    <div>
                      <Label className="text-gray-300">Battle Name</Label>
                      <Input
                        placeholder="Ex: JS Master Clash"
                        value={battleName}
                        onChange={(e) => setBattleName(e.target.value)}
                        className="mt-2 bg-[#1e1e1e] text-white border-gray-600"
                      />
                    </div>
                    ...
                    <Button
                      ...
                      disabled={loading}
                      className="w-full py-3 mt-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg"
                    >
                      {loading ? (
                        <>
                  ...
                  <div className="text-center py-6">
                    <h3 className="text-2xl font-bold text-green-400">Battle Created!</h3>
                    ...
                    <Button
                      ...
                      className="mt-5 border-gray-500 text-gray-200 hover:bg-gray-800"
                    >
                      Create Another
                    </Button>
                  </div>
          ...
          >
            <Card className="bg-transparent shadow-none">
              ...
              <CardContent className="space-y-5">
                ...
                <div>
                  <Label className="text-gray-300">Battle Code</Label>
                  <Input
                ...
                <Button
                  ...
                  disabled={loading}
                  className="w-full py-3 mt-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg"
                >
                  {loading ? (
                    <>
                      <Loader2 className="mr-2 h-5 w-5 animate-spin" />
        ...
        <motion.div
          ...
          className="mt-12 rounded-2xl border border-gray-700 bg-[#141414] p-6"
        >
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-2xl font-bold text-white">🔥 Recent Battles</h2>
            <p className="text-sm text-gray-400">{recentLoading ? "Loading..." : `${recentBattles.length} available`}</p>
          </div>

          {recentBattles.length === 0 && !recentLoading ? (
            <p className="text-gray-400 italic">No battles available.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {recentBattles.map((battle) => (
                <div
                  key={battle._id}
                  className="bg-[#1a1a1a] border border-gray-700 p-5 rounded-xl hover:bg-[#222] transition"
                >
                  <h3 className="text-white text-xl font-semibold">{safe(battle.battleName, "Untitled")}</h3>
                  <p className="text-gray-400 text-sm mt-1">Code: <span className="text-indigo-400">{safe(battle.battleCode)}</span></p>
                  <p className="text-gray-400 text-sm">Players: {Array.isArray(battle.players) ? battle.players.length : safe(battle.players, 0)}</p>
                  <p className="text-gray-400 text-sm">Tags: <span className="text-indigo-400">{Array.isArray(battle.tags) ? battle.tags.join(", ") : safe(battle.tags)}</span></p>
